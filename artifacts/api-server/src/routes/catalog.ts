...
router.get("/catalog/container-types", requireAuth, async (_req, res): Promise<void> => {
  const rows = await db.select().from(containerTypesTable).orderBy(containerTypesTable.name);
  res.json(rows);
});

router.post("/catalog/container-types", requireAuth, requirePermission(PERM.CATALOG_MANAGE), async (req, res): Promise<void> => {
...
router.put("/catalog/container-types/:id", requireAuth, requirePermission(PERM.CATALOG_MANAGE), async (req, res): Promise<void> => {
  ...
  const [row] = await db.update(containerTypesTable)
    ...
    .where(eq(containerTypesTable.id, id)).returning();
  if (!row) { res.status(404).json({ error: "Não encontrado" }); return; }
  res.json(row);
});

router.delete("/catalog/container-types/:id", requireAuth, requirePermission(PERM.CATALOG_MANAGE), async (req, res): Promise<void> => {
  const id = Number(req.params["id"]);
...
router.get("/catalog/capsule-types", requireAuth, async (_req, res): Promise<void> => {
  const rows = await db.select().from(capsuleTypesTable).orderBy(capsuleTypesTable.name);
  res.json(rows);
});

router.post("/catalog/capsule-types", requireAuth, requirePermission(PERM.CATALOG_MANAGE), async (req, res): Promise<void> => {
  const { name, description } = req.body as { name?: string; description?: string };
  if (!name?.trim()) { res.status(400).json({ error: "name obrigatório" }); return; }
...
router.put("/catalog/capsule-types/:id", requireAuth, requirePermission(PERM.CATALOG_MANAGE), async (req, res): Promise<void> => {
  const id = Number(req.params["id"]);
  const { name, description } = req.body as { name?: string; description?: string };
  if (!name?.trim()) { res.status(400).json({ error: "name obrigatório" }); return; }
  const [row] = await db.update(capsuleTypesTable)
    .set({ name: name.trim(), description: description?.trim() ?? null, updatedAt: new Date() })
    .where(eq(capsuleTypesTable.id, id)).returning();
  if (!row) { res.status(404).json({ error: "Não encontrado" }); return; }
  res.json(row);
});

router.delete("/catalog/capsule-types/:id", requireAuth, requirePermission(PERM.CATALOG_MANAGE), async (req, res): Promise<void> => {
  const id = Number(req.params["id"]);
...
router.get("/catalog/product-types", requireAuth, async (_req, res): Promise<void> => {
  const rows = await db.select().from(productTypesTable).orderBy(productTypesTable.name);
  res.json(rows);
});

router.post("/catalog/product-types", requireAuth, requirePermission(PERM.CATALOG_MANAGE), async (req, res): Promise<void> => {
...
router.put("/catalog/product-types/:id", requireAuth, requirePermission(PERM.CATALOG_MANAGE), async (req, res): Promise<void> => {
  const id = Number(req.params["id"]);
  const { name, description, isPowder } = req.body as { name?: string; description?: string; isPowder?: boolean };
  if (!name?.trim()) { res.status(400).json({ error: "name obrigatório" }); return; }
...
router.delete("/catalog/product-types/:id", requireAuth, requirePermission(PERM.CATALOG_MANAGE), async (req, res): Promise<void> => {
  const id = Number(req.params["id"]);
